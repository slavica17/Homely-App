...
import { getRoommateAd } from "@/data/roommates";
...
const RoommateDetails = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const result = await getRoommateAd(id);
      if (result.ok) {
        setAd(result.data);
      }
      setLoading(false);
  ...
  const typeLabel = (t) => (t === "NUDIM" ? "Offering a place" : "Looking for a place");
  ...
  if (!ad) {
    return (
      <Page>
        <Content>
          <Typography>Ad not found.</Typography>
        </Content>
      </Page>
    );
  }
  ...
  return (
    <Page>
      ...
      <Content>
        {images.length > 0 ? (
          <>
            <MainImage src={baseUrl + images[selectedImage]} alt={ad.title} />
            {images.length > 1 && (
              <Thumbs>
        ...
        <InfoCard>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Typography sx={{ fontSize: 28, fontWeight: 700 }}>{ad.title}</Typography>
            <Chip
          ...
          >
            ...
            {ad.authorEmail && (
              <Button
                ...
                href={`mailto:${ad.authorEmail}?subject=Regarding your roommate ad: ${ad.title}`}
                sx={{
